 ...
 */
export const getServiceImage = (serviceId, imageNumber = 1, extension = 'webp') => {
	if (!serviceId) {
		return `/images/service/service-1.webp`; // fallback
	}
	
	// Поддержка текущей структуры для сервиса ID 1 (папка serivce-1 с файлами "1 (1).webp")
	if (serviceId === 1) {
 ...
 */
export const getServiceImages = (serviceId, count = 10, extension = 'webp') => {
	if (!serviceId) {
		return [];
	}
 ...
 */
export const getAvailableServiceImages = async (serviceId, maxCount = 10, extension = 'webp') => {
	if (!serviceId) {
		return [];
	}
...
export const getServiceMainImage = (serviceId, fallback = '/images/service/service-details.webp') => {
	...
	return getServiceImage(serviceId, 1, 'webp');
};
